...
class Dblock(nn.Module):
  def __init__(self,in_channels,sample_factor=2,kernel=3,stride=2,atten=False):
    ...
    self.residual2=Residual_block(in_channels,atten=False)
    if in_channels//4>0:
      self.inn=in_channels//4
    else:
      self.inn=1
  ...
  def forward(self,x,t=0):
    x=self.residual(x,t)+x
    #x=self.residual2(x,t)+x
    return self.conv1(self.gn(x))
...
class Unet(nn.Module):
  def __init__(self,in_channels,f_initial,sample_factor=2,kernel=3,atten=True):
    ...
    if atten:
      ...
      self.dblocks = nn.ModuleList([Dblock(f_initial * (2**i), atten=(i > 100)) for i in range(0,3)])
      self.ublocks = nn.ModuleList([Ublock(f_initial * (2**i), atten=(i > 200)) for i in range(3,0,-1)])
    self.residual=list(range(4))
    self.residual_block=Residual_block(f_initial*(2**3),atten=atten)
...
def test_unet():
    ...
    input_tensor = torch.randn(2, 1, 31, 31)

    # Instantiate the Unet model
    unet_model = Unet(in_channels=1, f_initial=8)
    ...
    assert output_tensor.shape == (2, 1, 31, 31)
